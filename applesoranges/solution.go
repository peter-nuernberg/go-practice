...
func countApplesAndOranges(s int32, t int32, a int32, b int32, apples []int32, oranges []int32) {
	fmt.Println(count(s, t, shift(a, apples)))
	fmt.Println(count(s, t, shift(b, oranges)))
}

func shift(center int32, offsets []int32) []int32 {
	var shifted[]int32
	for i := 0; i < len(offsets); i++ {
		shifted = append(shifted, center + offsets[i])
	}
	return shifted
}

func count(s int32, t int32, landingSpots []int32) int32 {
	var count int32 = 0
	for i := 0; i < len(landingSpots); i++ {
		if landingSpots[i] >= s && landingSpots[i] <= t {
			count++
		}
	}
	return count
